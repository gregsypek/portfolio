...
const createBox = function (array) {
  for (let i = 0; i < array.length; i++) {
    ...
    box.classList.add(`projects__box--${i}`);
...
class Slider {
  constructor(images, box) {
    this.images = images;
    // this.slide = null;

    this.image = null;
    this.currentSlide = 0;
    this.slideArrayLength = images.length;
    this.box = box;
  }
  ...
  prevSlide(index) {
    if (index === 0) {
      ...
      index = this.slideArrayLength - 1;
    } else index--;
    this.currentSlide = index;
  }

  nextSlide(index) {
    if (index === this.slideArrayLength - 1) {
      index = 0;
      this.currentSlide = index;
    } else {
      index++;
...
const prevBtns = [...document.querySelectorAll('[data-move-prev]')];
console.log(prevBtns);

const nextBtns = [...document.querySelectorAll('[data-move-next]')];
console.log(nextBtns);

let projectNr = 0;
let slideImgNr = 0;

const prevSlide = function (slideImgNr, projectNr) {
  if (slideImgNr === 1) {
    slideImgNr = setsOfProjects[projectNr].images.length - 1;
  } else slideImgNr--;
  // currentSlide = slideImgNr;
  console.log(slideImgNr, projectNr);
};

changeSlide = function (index, projectNr) {
  // if (index === -1 || index === projectNr) return;
  this.currentSlide = index;
  this.setSlideAttributes(index);
};
...
prevBtns.forEach(btn => {
  btn.addEventListener('click', function (e) {
    const clicked = e.target.closest('.projects__icon--move');
    console.log(clicked);
    projectNr = Number([...clicked.classList[2]].splice(-1, 1));

    slideImgNr = Number(
      boxes.children[projectNr]
        .querySelector('.projects__img')
        .getAttribute('alt')
        .slice(-1)
    );
    console.log(slideImgNr, projectNr);
    prevSlide(slideImgNr, projectNr);
  });
});
const boxes = document.querySelector('.projects__boxes');
console.log(boxes);

// const listeners = function (projectNr, imgNr) {
//   prevBtn.
// };
